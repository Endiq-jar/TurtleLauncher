...
# Gradle's own "What went wrong" block names the failure even when no task reached the
# compiler (configuration-time errors, dependency resolution, plugin problems).
WW=$(grep -n '^What went wrong:' "$LOG" | head -n1 | cut -d: -f1 || true)
if [ -n "$WW" ]; then
  emit error "what-went-wrong:
$(sed -n "$((WW + 1)),$((WW + 25))p" "$LOG" | grep -vE '^\s*$' | cut -c1-400)"
fi

# Kotlin/aapt/AGP per-file diagnostics (compile errors land here, not in the task window).
DIAG=$(grep -nE '^(e|w): file|AAPT: error|Android resource linking failed|Unresolved reference|Overload resolution ambiguity|error: cannot find symbol|extracting|Could not (create|initialize)' "$LOG" | head -10)
[ -n "$DIAG" ] && emit error "diagnostics:
$DIAG"
