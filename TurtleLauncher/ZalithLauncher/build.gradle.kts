...
dependencies {
    ...
    // Faster pack/world I/O (zstd compression). Native lib bundled in jniLibs/arm64-v8a;
    // version pinned to match that native build exactly (zstd-jni's Native.load() requires
    // an exact version string match between the jar and the .so it finds).
