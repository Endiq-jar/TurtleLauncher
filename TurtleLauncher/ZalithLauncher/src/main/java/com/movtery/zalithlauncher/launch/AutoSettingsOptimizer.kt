 ...
 *
 * Renderer/driver selection logic is unchanged from AutoGraphicsOptimizer:
 *   Adreno GPU     → renderer = gl4es114, driver = Turnip (if Vulkan available)
 *   Mali GPU       → renderer = opengles3, driver = default
 *   PowerVR/other  → renderer = opengles2, driver = default
 ...
 * on those GPUs we switch to VulkanZink if Vulkan is available, which
...
object AutoSettingsOptimizer {
    ...
    private fun applyGraphics(context: Context, mcVersionId: String) {
        ...
        val (renderer, driver) = when {
            ...
            gpu.contains("apple", ignoreCase = true) -> {
                if (isMC26Plus && hasVulkan) {
                    Logging.i(TAG, "MC 26.1+ on PowerVR/Apple GPU: switching to VulkanZink to avoid CubeMap GL_INVALID_ENUM crash")
                    Pair("vulkan_zink", "default")
                } else {
                    Pair("opengles2", "default")
                }
            }
            else -> {
                if (isMC26Plus && hasVulkan) {
                    Logging.i(TAG, "MC 26.1+ unknown GPU: attempting VulkanZink to avoid CubeMap GL_INVALID_ENUM crash")
                    Pair("vulkan_zink", "default")
                } else {
        ...
        Logging.i(TAG, "Auto-selected renderer=$renderer driver=$driver")
        AllSettings.renderer.put(renderer).save()
