...
class VideoSettingsFragment : AbstractSettingsFragment(R.layout.settings_fragment_video, SettingCategory.VIDEO) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // GL4ESRenderer's UUID, Vulkan maps to VulkanZinkRenderer's UUID (Zink translates
        // Minecraft's GL calls to Vulkan). Falls back to whichever of the two is actually
        // compatible/present in the device's renderer list, in case one isn't available.
        val openglRendererId = renderers.rendererIdentifier.getOrNull(
            renderers.rendererNames.indexOfFirst { it.equals("GL4ES", ignoreCase = true) }
        )
        val vulkanRendererId = renderers.rendererIdentifier.getOrNull(
            renderers.rendererNames.indexOfFirst { it.contains("Zink", ignoreCase = true) || it.contains("Vulkan", ignoreCase = true) }
        )
