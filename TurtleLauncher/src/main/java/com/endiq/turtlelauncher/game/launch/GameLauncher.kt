...
import com.endiq.turtlelauncher.bridge.LoggerBridge.appendInfo
import com.endiq.turtlelauncher.bridge.LoggerBridge.appendTitle
import com.endiq.turtlelauncher.bridge.TLBridge
import com.endiq.turtlelauncher.context.readAssetFile
import com.endiq.turtlelauncher.game.account.Account
import com.endiq.turtlelauncher.game.account.AccountType
import com.endiq.turtlelauncher.game.account.offline.OfflineYggdrasilServer
...
import com.endiq.turtlelauncher.utils.file.ensureDirectorySilently
import com.endiq.turtlelauncher.utils.logging.Logger
import com.endiq.turtlelauncher.utils.string.isBiggerTo
import com.endiq.turtlelauncher.utils.string.isEqualTo
...
class GameLauncher(
    ...
    override fun initEnv(screenSize: IntSize): MutableMap<String, String> {
        ...
        envMap["TURTLE_VERSION_CODE"] = BuildConfig.VERSION_CODE.toString()
        return envMap
    }
    ...
    override fun dlopenEngine() {
        ...
        RendererPluginManager.selectedRendererPlugin?.let { renderer ->
            ...
            libs.forEach { libPath ->
                TLBridge.dlopen(libPath)
            }
        }

        val rendererLib = getRendererLibrary() ?: return
        if (!TLBridge.dlopen(rendererLib) && !TLBridge.dlopen(findInLdLibPath(rendererLib))) {
    ...
    private fun printLauncherInfo(
        ...
        appendInfo("Architecture: ${Architecture.archAsString(TLApplication.DEVICE_ARCHITECTURE)}")
...
private fun setRendererEnv(envMap: MutableMap<String, String>) {
    ...
    if (renderer != GL4ESRenderer && renderer != NGGL4ESRenderer && renderer != NWRenderer &&
        renderer != LTWRenderer && renderer != MobileGluesRenderer
    ) {
