...
import com.endiq.turtlelauncher.components.Components
import com.endiq.turtlelauncher.components.UnpackComponentsTask
import com.endiq.turtlelauncher.context.GlobalContext
import com.endiq.turtlelauncher.game.multirt.Runtime
...
import com.endiq.turtlelauncher.path.LibPath
...
import com.endiq.turtlelauncher.utils.device.Architecture.ARCH_X86
import com.endiq.turtlelauncher.utils.device.Architecture.is64BitsDevice
import com.endiq.turtlelauncher.utils.logging.Logger
import com.endiq.turtlelauncher.utils.network.getSystemDnsServerAddresses
import com.endiq.turtlelauncher.utils.string.getMessageOrToString
import com.endiq.turtlelauncher.utils.string.splitPreservingQuotes
...
abstract class Launcher(
    ...
    protected suspend fun launchJvm(
        ...
        TLNativeInvoker.staticLauncher = this

        TLBridge.setLdLibraryPath(getRuntimeLibraryPath())
    ...
    private suspend fun launchJavaVM(
        ...
        TLBridge.setupExitMethod(context.applicationContext)
        TLBridge.initializeGameExitHook()
        TLBridge.chdir(chdir())
    ...
    private fun setJavaEnv(
        ...
        envMap().let { map ->
            ...
            map["DALVIK_JAVAVM"] = TLBridge.getJavaVMPointer().toString()
            map["DALVIK_APPLICATION"] = TLBridge.jObjectToString(GlobalContext.applicationContext)
    ...
    private fun dlopenJavaRuntime() {
        ...
        TLBridge.dlopen("$jliLibDir/libjli.so")
        TLBridge.dlopen("$jvmLibDir/libjvm.so")
        TLBridge.dlopen("$javaLibDir/libfreetype.so")
        TLBridge.dlopen("$javaLibDir/libverify.so")
        TLBridge.dlopen("$javaLibDir/libjava.so")
        TLBridge.dlopen("$javaLibDir/libnet.so")
        TLBridge.dlopen("$javaLibDir/libnio.so")
        TLBridge.dlopen("$javaLibDir/libawt.so")
        TLBridge.dlopen("$javaLibDir/libawt_headless.so")
        TLBridge.dlopen("$javaLibDir/libfontmanager.so")
        locateLibs(File(runtimeHome)).forEach { file ->
            TLBridge.dlopen(file.absolutePath)
        }
    }

    @CallSuper
    protected open fun dlopenEngine() {
        TLBridge.dlopen("${PathManager.DIR_NATIVE_LIB}/libopenal.so")
    }
}
