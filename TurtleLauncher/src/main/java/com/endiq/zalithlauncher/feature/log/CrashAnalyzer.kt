package com.endiq.zalithlauncher.feature.log
...
object CrashAnalyzer {
    ...
    private val rules: List<Rule> by lazy {
        listOf(
            ...
            Rule(
                ...
                matches = {
                    com.endiq.zalithlauncher.setting.AllSettings.fastBoot.getValue() &&
                        has(it, "zip END header not found", "invalid LOC header", "ZipException", "Truncated ZIP file",
    ...
    private fun recordRuleFired(ruleTitle: String) {
        runCatching {
            val ctx = com.endiq.zalithlauncher.context.ContextExecutor.getApplication()
    ...
    fun getCustomRules(): List<CustomRule> {
        return runCatching {
            val json = com.google.gson.JsonParser.parseString(
                com.endiq.zalithlauncher.setting.AllSettings.customCrashRules.getValue()
            ).asJsonArray
            json.mapNotNull { el ->
    ...
    private fun saveCustomRules(rules: List<CustomRule>) {
        ...
        com.endiq.zalithlauncher.setting.AllSettings.customCrashRules.put(array.toString()).save()
    }
    ...
    fun getCrashHistory(): List<CrashHistoryEntry> {
        return runCatching {
            val json = com.google.gson.JsonParser.parseString(
                com.endiq.zalithlauncher.setting.AllSettings.crashHistoryList.getValue()
            ).asJsonArray
            json.mapNotNull { el ->
                if (!el.isJsonObject) return@mapNotNull null
                val obj = el.asJsonObject
    ...
    private fun pushCrashHistory(exitCode: Int, summary: String) {
        ...
        com.endiq.zalithlauncher.setting.AllSettings.crashHistoryList.put(array.toString()).save()
    }

    @JvmStatic
    fun clearCrashHistory() {
        com.endiq.zalithlauncher.setting.AllSettings.crashHistoryList.put("[]").save()
    }
     ...
     * pre-launch warning dialog (see [com.endiq.zalithlauncher.feature.mod.ModAutoMaintenance]).
     */
    private fun modConflictDiagnoses(gameVersion: Version?): List<Diagnosis> {
        ...
        return runCatching {
            ...
            com.endiq.zalithlauncher.feature.mod.ModConflictDetector.detectConflicts(modsFolder)
                .map { conflict ->
                    Diagnosis(
