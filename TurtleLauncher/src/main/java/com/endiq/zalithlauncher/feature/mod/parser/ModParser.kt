package com.endiq.zalithlauncher.feature.mod.parser
        ...
        return cacheFile.takeIf { it.exists() }?.let {
            runCatching {
                gson.fromJson<List<ModInfoCache>>(
                    com.endiq.zalithlauncher.utils.CacheCompression.readCompressed(it),
    ...
    private fun persistCache(gson: Gson, cacheFile: File, data: List<ModInfoCache>) {
        Task.runTask {
            runCatching {
                ...
                com.endiq.zalithlauncher.utils.CacheCompression.writeCompressed(cacheFile, gson.toJson(data))
            }.onFailure { e ->
