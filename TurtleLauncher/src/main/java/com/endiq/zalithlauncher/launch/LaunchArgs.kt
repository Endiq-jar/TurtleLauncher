...
import com.endiq.zalithlauncher.feature.customprofilepath.ProfilePathHome.Companion.getLibrariesHome
...
class LaunchArgs(
    ...
    fun getAllArgs(): List<String> {
        ...
        if (Tools.resolveLwjglMode(versionInfo) == Tools.LwjglMode.NEW_SDL) {
            ...
            argsList.add("com.endiq.zalithlauncher.launch.SdlMainReadyBootstrap")
        } else {
    ...
    private fun getJavaArgs(): List<String> {
        ...
        if (AccountUtils.isOtherLoginAccount(account)) {
        ...
        } else if (account.accountType == com.endiq.zalithlauncher.feature.accounts.AccountType.LOCAL.type &&
            com.endiq.zalithlauncher.setting.AllSettings.localSkinServerEnabled.getValue()
        ) {
            ...
            val port = com.endiq.zalithlauncher.feature.skin.TurtleSkinServer.ensureStarted(
                com.endiq.zalithlauncher.setting.AllSettings.localSkinServerLanVisible.getValue()
            )
            if (port > 0 && LibPath.AUTHLIB_INJECTOR.isFile) {
                val apiRoot = com.endiq.zalithlauncher.feature.skin.TurtleSkinServer.apiRootUrl()
