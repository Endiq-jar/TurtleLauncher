package com.endiq.zalithlauncher.plugins
...
import com.endiq.zalithlauncher.plugins.feature.FeaturePluginManager
...
object PluginLoader {
    ...
    fun loadAllPlugins(context: Context, force: Boolean = false) {
        ...
        if (force) FeaturePluginManager.clearPlugin()
        ...
        if (com.endiq.zalithlauncher.setting.AllSettings.autoCheckPluginUpdates.getValue() &&
            !com.endiq.zalithlauncher.setting.AllSettings.fastBoot.getValue() &&
            !com.endiq.zalithlauncher.task.TaskExecutors.isGameSessionActive) {
            com.endiq.zalithlauncher.feature.pluginupdate.PluginUpdateManager.checkForUpdates(context, force = false) { updates, _ ->
                if (updates.isEmpty()) return@checkForUpdates

                com.endiq.zalithlauncher.feature.log.Logging.i(
                    "PluginLoader",
                ...
                autoInstallable.forEach { asset ->
                    com.endiq.zalithlauncher.feature.pluginupdate.PluginUpdateManager.downloadAndInstall(context, asset) { success, message ->
                        com.endiq.zalithlauncher.feature.log.Logging.i("PluginLoader", "Auto-update ${asset.assetName}: $message")
                    }
                }
            }
        }
