...
import com.endiq.zalithlauncher.ui.activity.ConfigEditorActivity
import com.endiq.zalithlauncher.R
import com.endiq.zalithlauncher.databinding.FragmentFilesBinding
import com.endiq.zalithlauncher.event.sticky.FileSelectorEvent
import com.endiq.zalithlauncher.feature.download.utils.ModLoaderUtils
import com.endiq.zalithlauncher.feature.mod.ModUpdateChecker
import com.endiq.zalithlauncher.feature.turtle.heatmap.HeatmapAnalyzer
...
import com.endiq.zalithlauncher.utils.file.ZipExtractUtils
...
class FilesFragment : FragmentWithAnim(R.layout.fragment_files) {
    ...
    private fun analyzeHeatmap(folder: File) {
        ...
        val adapter = binding.fileRecyclerView.adapter ?: return
        val items = adapter.data.filter { it.file != null }
        if (items.isEmpty()) return

        Task.runTask {
            items.mapNotNull { item ->
                val file = item.file ?: return@mapNotNull null
                HeatmapAnalyzer.analyze(file, category)?.let { estimate -> item to estimate }
            }
        }.ended(TaskExecutors.getAndroidUI()) { results ->
            var changed = false
            results.orEmpty().forEach { (item, estimate) ->
                item.perfEstimate = estimate
                changed = true
            }
            if (changed) adapter.notifyDataSetChanged()
        }.execute()
