...
import com.endiq.zalithlauncher.databinding.SettingsFragmentExperimentalBinding
import com.endiq.zalithlauncher.feature.pluginupdate.PluginUpdateManager
...
class ExperimentalSettingsFragment :
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        BaseSettingsWrapper(context, binding.exportSettingsLayout) {
            ...
            val ok = com.endiq.zalithlauncher.feature.turtle.SettingsBackupManager.exportToFile(destFile)
            Toast.makeText(
                context,
        ...
        BaseSettingsWrapper(context, binding.importSettingsLayout) {
            ...
            val ok = com.endiq.zalithlauncher.feature.turtle.SettingsBackupManager.importFromFile(srcFile)
            Toast.makeText(
                context,
        ...
        BaseSettingsWrapper(context, binding.crashHistoryLayout) {
            val history = com.endiq.zalithlauncher.feature.log.CrashAnalyzer.getCrashHistory()
            ...
            com.endiq.zalithlauncher.ui.dialog.TipDialog.Builder(context)
                ...
                .setCancelClickListener { com.endiq.zalithlauncher.feature.log.CrashAnalyzer.clearCrashHistory() }
        ...
        SwitchSettingsWrapper(
            context,
            com.endiq.zalithlauncher.setting.AllSettings.aiCrashHelpEnabled,
        ...
        BaseSettingsWrapper(context, binding.dependencyGraphLayout) {
            val version = com.endiq.zalithlauncher.feature.version.VersionsManager.getCurrentVersion()
            if (version == null) {
                ...
                Thread {
                    val graphText = com.endiq.zalithlauncher.feature.turtle.DependencyGraphExporter.buildTextTree(version)
                    com.endiq.zalithlauncher.task.TaskExecutors.runInUIThread {
                        com.endiq.zalithlauncher.ui.dialog.TipDialog.Builder(context)
        ...
        BaseSettingsWrapper(context, binding.configEditorLayout) {
            ...
            if (version == null) {
                ...
                val intent = android.content.Intent(context, com.endiq.zalithlauncher.ui.activity.ConfigEditorActivity::class.java)
                intent.putExtra(com.endiq.zalithlauncher.ui.activity.ConfigEditorActivity.EXTRA_VERSION_NAME, version.getVersionName())
        ...
        BaseSettingsWrapper(
            ...
            val snapshot = com.endiq.zalithlauncher.feature.log.CrashAnalyzer.getTelemetrySnapshot(context)
    ...
    private fun promptSetAiCrashHelpApiKey(context: android.content.Context) {
        val current = runCatching { com.endiq.zalithlauncher.setting.AllSettings.aiApiKey.getValue() }.getOrDefault("")
        com.endiq.zalithlauncher.ui.dialog.EditTextDialog.Builder(context)
            ...
            .setConfirmListener { editText, _ ->
                com.endiq.zalithlauncher.setting.AllSettings.aiApiKey.put(editText.text.toString().trim()).save()
    ...
    private fun showCrashRuleEditor(context: android.content.Context) {
        val rules = com.endiq.zalithlauncher.feature.log.CrashAnalyzer.getCustomRules()
    ...
    private fun promptAddCrashRuleTip(context: android.content.Context, pattern: String) {
        com.endiq.zalithlauncher.ui.dialog.EditTextDialog.Builder(context)
            ...
            .setAsRequired()
            .setConfirmListener { editText, _ ->
                com.endiq.zalithlauncher.feature.log.CrashAnalyzer.addCustomRule(pattern, editText.text.toString())
                Toast.makeText(context, R.string.generic_ok, Toast.LENGTH_SHORT).show()
                showCrashRuleEditor(context)
                true
            }
