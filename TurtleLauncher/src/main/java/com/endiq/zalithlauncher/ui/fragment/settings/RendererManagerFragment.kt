...
import com.endiq.zalithlauncher.databinding.FragmentRendererManagerBinding
import com.endiq.zalithlauncher.feature.download.RendererCardAdapter
import com.endiq.zalithlauncher.feature.pluginupdate.PluginAssetInfo
import com.endiq.zalithlauncher.feature.pluginupdate.PluginKind
...
class RendererManagerFragment : FragmentWithAnim(R.layout.fragment_renderer_manager) {
    ...
    private fun buildCompatNote(
        renderer: com.endiq.zalithlauncher.renderer.RendererInterface,
        catalogEntry: RendererCatalog.Entry?,
        hasVulkan: Boolean
    ): String? {
        val libMissing = !File(com.endiq.zalithlauncher.utils.path.PathManager.DIR_NATIVE_LIB, renderer.getRendererLibrary()).exists()
        if (libMissing) return getString(R.string.renderer_compat_missing_library)

        if (renderer.getRendererId() == com.endiq.zalithlauncher.renderer.renderers.ZinkRenderer.ID && !hasVulkan) {
