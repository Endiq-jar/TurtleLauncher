package com.endiq.zalithlauncher.utils;
...
public final class ZHTools {
    ...
    public synchronized static Drawable customMouse(Context context) {
        ...
        return com.endiq.zalithlauncher.feature.turtle.cursor.CustomCursorLoader.INSTANCE.load(context, mouseFile);
    }
    ...
    public static synchronized void shareLogs(Context context) {
        ...
        Task.runTask(() -> {
                    ...
                    try (FileOutputStream fos = new FileOutputStream(zipFile);
                        ...
                        // Raw stdout/stderr capture of the last game process the launcher ran
                        // (not the same thing as Minecraft's own logs/latest.log below - this
                        // is everything printed to the JVM's stdout/stderr, launcher-side
                        // messages included).
                        ...
                        if (currentVersion != null) {
                            ...
                            File mcLogsFolder = new File(gameDir, "logs");
                            if (mcLogsFolder.exists() && mcLogsFolder.isDirectory()) {
                                FileTools.zipDirectory(mcLogsFolder, "minecraft_logs/", file -> true, zos);
                            } else Log.d("Zip Log", "Minecraft's own logs folder does not exist for the current version");

                            File mcCrashReportsFolder = new File(gameDir, "crash-reports");
                            if (mcCrashReportsFolder.exists() && mcCrashReportsFolder.isDirectory()) {
                                FileTools.zipDirectory(mcCrashReportsFolder, "minecraft_crash_reports/", file -> true, zos);
                            } else Log.d("Zip Log", "Minecraft's own crash-reports folder does not exist for the current version");
                        } else Log.d("Zip Log", "No current version selected - skipping Minecraft's own logs");
                    }
