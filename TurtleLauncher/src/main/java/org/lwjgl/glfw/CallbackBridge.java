...
import static com.endiq.turtlelauncher.bridge.TLBridgeStatesKt.CURSOR_DISABLED;
import static com.endiq.turtlelauncher.bridge.TLBridgeStatesKt.CURSOR_ENABLED;
...
public class CallbackBridge {
    ...
    private static final Consumer<Boolean> grabListener = isGrabbing ->
            TLBridgeStates.changeCursorMode(isGrabbing ? CURSOR_DISABLED : CURSOR_ENABLED);

    private static int cursorShape = GLFW_ARROW_CURSOR;
    private static final Consumer<CursorShape> cursorShapeListener = TLBridgeStates::changeCursorShape;
    ...
    public static boolean notifyLauncher(int type, int... action) {
        if (action == null || action.length == 0) {
            LoggerBridge.append("TurtleLauncher: SDL notification has no action");
            return false;
        }
        switch (type) {
            case NOTIF_TYPE_SDL:
                if (action[0] == ACTION_INIT_LAUNCHER_INTEGRATION) {
                    ...
                    try {
                        LoggerBridge.append("TurtleLauncher: loading SDL3");
                        System.loadLibrary("SDL3");
                        LoggerBridge.append("TurtleLauncher: loading SDL2");
                        System.loadLibrary("SDL2");
                        LoggerBridge.append("TurtleLauncher: setting up SDL JNI");
                        SdlBridge.setupJNI();
                        LoggerBridge.append("TurtleLauncher: binding SDL surface");
                        ...
                        LoggerBridge.append("TurtleLauncher: SDL support enabled!");
                        return true;
                    } catch (Throwable e) {
                        ...
                        LoggerBridge.append("TurtleLauncher: SDL launcher integration is unavailable:\n" + trace);
                    }
                }
    ...
    private static void onDirectInputEnable() {
        LoggerBridge.append("TurtleLauncher: Direct gamepad input enabled");
