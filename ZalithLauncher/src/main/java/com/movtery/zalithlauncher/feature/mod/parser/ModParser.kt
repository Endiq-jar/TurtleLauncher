        ...
        return cacheFile.takeIf { it.exists() }?.let {
            runCatching {
                gson.fromJson<List<ModInfoCache>>(
                    com.movtery.zalithlauncher.utils.CacheCompression.readCompressed(it),
    ...
    private fun persistCache(gson: Gson, cacheFile: File, data: List<ModInfoCache>) {
        Task.runTask {
            runCatching {
                ...
                com.movtery.zalithlauncher.utils.CacheCompression.writeCompressed(cacheFile, gson.toJson(data))
            }.onFailure { e ->
