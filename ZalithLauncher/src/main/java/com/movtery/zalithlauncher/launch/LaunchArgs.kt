...
class LaunchArgs(
    ...
    fun getAllArgs(): List<String> {
        ...
        if (Tools.resolveLwjglMode(versionInfo) == Tools.LwjglMode.NEW_SDL) {
            ...
            argsList.add("com.movtery.zalithlauncher.launch.SdlMainReadyBootstrap")
        } else {
    ...
    private fun getJavaArgs(): List<String> {
        ...
        if (AccountUtils.isOtherLoginAccount(account)) {
        ...
        } else if (account.accountType == com.movtery.zalithlauncher.feature.accounts.AccountType.LOCAL.type &&
            com.movtery.zalithlauncher.setting.AllSettings.localSkinServerEnabled.getValue()
        ) {
            ...
            val port = com.movtery.zalithlauncher.feature.skin.TurtleSkinServer.ensureStarted(
                com.movtery.zalithlauncher.setting.AllSettings.localSkinServerLanVisible.getValue()
            )
            if (port > 0 && LibPath.AUTHLIB_INJECTOR.isFile) {
                val apiRoot = com.movtery.zalithlauncher.feature.skin.TurtleSkinServer.apiRootUrl()
