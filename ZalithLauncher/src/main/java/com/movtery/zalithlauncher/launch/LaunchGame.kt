...
class LaunchGame {
    companion object {
        ...
        private fun launch(
            ...
            // TurtleLauncher: VirGL (see VirglRenderer/VirglTestServerManager) needs its own
            // background process serving the vtest transport for the whole time the game is
            // running - start it right before the blocking launch call, guarantee it's torn
            // down after, regardless of how the game exits.
            val usingVirgl = Renderers.getCurrentRenderer().getRendererId() == "virgl_vtest"
            if (usingVirgl) VirglTestServerManager.start()
            try {
                JREUtils.launchWithUtils(activity, runtime, minecraftVersion, launchArgs, customArgs)
            } finally {
                if (usingVirgl) VirglTestServerManager.stop()
            }
        }
