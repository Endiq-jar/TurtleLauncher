...
import com.movtery.zalithlauncher.renderer.RendererFallbackManager
...
class LaunchGame {
    companion object {
        ...
        fun runGame(activity: AppCompatActivity, minecraftVersion: Version, version: JMinecraftVersionList.Version) {
            ...
            // TurtleLauncher: automatic renderer fallback. If the last launch attempt with
            // the currently-selected renderer never exited cleanly, try the next one in the
            // spec's fallback chain for this attempt instead. See RendererFallbackManager's
            // doc comment for exactly what this can/can't do (it's a next-launch retry, not
            // a same-session recovery - nothing can recover a process that already crashed).
            val hasSodiumOrEmbeddium = runCatching {
                File(minecraftVersion.getGameDir(), "mods").listFiles()
                    ?.any { it.name.contains("sodium", ignoreCase = true) || it.name.contains("embeddium", ignoreCase = true) } == true
            }.getOrDefault(false)
            RendererFallbackManager.resolveForThisLaunch(activity, version.id ?: "", hasSodiumOrEmbeddium)?.let { fallbackUuid ->
                Renderers.setCurrentRenderer(activity, fallbackUuid)
                AllSettings.renderer.put(fallbackUuid).save()
            }
            // Recorded optimistically before launch, cleared on a clean exit (JREUtils) -
            // if the JVM never gets the chance to log a clean exit at all, this stays true
            // and the next launch attempt knows to fall back.
            AllSettings.lastLaunchRendererId.put(Renderers.getCurrentRenderer().getUniqueIdentifier()).save()
            AllSettings.lastLaunchFailed.put(true).save()
            ...
            val customArgs = (baseArgs + " " + buildFpsBoostArgs()).trim()
        ...
        private fun getRuntime(activity: Activity, version: Version, targetJavaVersion: Int): String {
            ...
            val autoInstalled = TurtleJREAutoInstaller.ensureJavaInstalled(activity, targetJavaVersion)
         ...
         */
        private fun buildFpsBoostArgs(): String {
            val args = mutableListOf<String>()

            if (AllSettings.unlimitedFps.getValue()) {
                // Remove GC-triggered explicit collections and drop the 300-FPS V-sync cap
                args += "-XX:+DisableExplicitGC"
                args += "-Dfml.earlyprogresswindow=false"
            }

            if (AllSettings.lowLatencyRendering.getValue()) {
                // Reduce per-frame string/compile overhead on mobile GPUs
                args += "-XX:+UseStringDeduplication"
                args += "-XX:+OptimizeStringConcat"
            }

            if (AllSettings.framePacing.getValue()) {
                // Enable LWJGL frame-pacing hints for smoother frame delivery
                args += "-Dorg.lwjgl.opengl.Display.noinput=false"
            }

            if (AllSettings.frameSkipping.getValue()) {
                // Allow the renderer to drop frames under overload instead of queuing
                args += "-Dorg.lwjgl.opengl.frameskip=true"
            }

            if (AllSettings.adaptiveFrameTiming.getValue()) {
                // Short GC pause target to avoid mid-frame stops
                args += "-XX:MaxGCPauseMillis=10"
                args += "-XX:GCTimeRatio=9"
            }
