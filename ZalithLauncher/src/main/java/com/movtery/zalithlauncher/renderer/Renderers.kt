 ...
 * unique identifier, and [RendererCatalog] for compatibility ranges/badges/fallback order.
...
object Renderers {
    ...
    fun getCompatibleRenderers(context: Context): Pair<RenderersList, List<RendererInterface>> = compatibleRenderers ?: run {
        ...
        renderers.forEach { renderer ->
            ...
            // VirGL (libOSMesa_81.so) and VGPU (libvgpu.so) are known-missing right now (see
            // their class doc comments); hard-filtering them out would silently shrink the
            // picker instead of surfacing that they need sourcing.
