...
class AllSettings {
    companion object {
        ...
        /** GL4ES-family renderers (Holy GL4ES, Krypton Wrapper) only: turns on libgl4es's own
         *  LIBGL_BATCH (batches GL calls instead of issuing them one at a time), LIBGL_USEVBO
         *  (routes vertex data through VBOs instead of client-side arrays), LIBGL_SKIPTEXCOPIES
         *  (skips a redundant texture copy libgl4es otherwise does on certain upload paths),
         *  and LIBGL_RECYCLEFBO (reuses framebuffer objects instead of recreating them) - all
         *  four confirmed present via `strings` on the actual bundled libgl4es_114/115.so, not
         *  assumed from general GL4ES docs. No equivalent for Zink/VirGL/VGPU/Freedreno since
         *  those never touch libgl4es. */
        @JvmStatic val gl4esPerformanceTweaks = BooleanSettingUnit("gl4esPerformanceTweaks", true)
        ...
        // fail the exact same way the moment it's pushed. Left blank - AiChatAdvisor/
        // AiCrashAdvisor already handle an empty key by prompting the user to add their own
