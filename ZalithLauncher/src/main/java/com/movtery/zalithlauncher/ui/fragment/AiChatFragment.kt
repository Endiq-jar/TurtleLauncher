...
import com.movtery.zalithlauncher.ui.subassembly.aichat.ChatResult
...
class AiChatFragment : FragmentWithAnim(R.layout.fragment_ai_chat) {
    ...
    private fun sendCurrentMessage() {
        ...
        scope.launch {
            val result = withContext(Dispatchers.IO) { AiChatAdvisor.sendMessage(history) }
            binding.thinkingIndicator.visibility = View.GONE
            binding.sendButton.isEnabled = true

            // Show the actual reason (HTTP code + Gemini's own error message, or the
            // exception) instead of a generic string, so a failure is diagnosable from
            // the app itself - see ChatResult.Failure.
            val replyMessage = ChatMessage(
                text = when (result) {
                    is ChatResult.Success -> result.text
                    is ChatResult.Failure -> getString(R.string.ai_chat_request_failed, result.reason)
                },
                isUser = false
            )
