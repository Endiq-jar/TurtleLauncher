...
public class MinecraftDownloader {
    ...
    private void extractNatives(String versionName) throws IOException {
        ...
        // TurtleLauncher Fast Boot: skip re-extraction entirely if a marker proves the
        // exact same set of source AARs (path + size + mtime) was already extracted here.
        File marker = new File(targetDirectory, ".tl_natives_marker");
        String signature = buildNativesSignature();
        if (AllSettings.getFastBoot().getValue() && marker.isFile()) {
            try {
                String existing = Tools.read(marker);
                if (signature.equals(existing)) {
                    Logging.i("NewMCDownloader", "Fast Boot: skipping natives re-extraction (marker matches)");
                    return;
                }
            } catch (IOException ignored) {
                // fall through and re-extract normally
            }
        }
        ...
        try {
            Tools.write(marker.getAbsolutePath(), signature);
        } catch (IOException e) {
            Logging.i("NewMCDownloader", "Failed to write natives extraction marker", e);
        }
    }

    private String buildNativesSignature() {
    ...
    private File downloadGameJson(JMinecraftVersionList.Version verInfo) throws IOException, MirrorTamperedException {
        ...
        try {
            DownloadUtils.ensureSha1(targetFile, (AllSettings.getVerifyManifest().getValue() && !AllSettings.getFastBoot().getValue()) ? verInfo.sha1 : null, () -> {
                ProgressLayout.setProgress(ProgressLayout.DOWNLOAD_MINECRAFT, 0,
    ...
    private JAssets downloadAssetsIndex(JMinecraftVersionList.Version verInfo) throws IOException{
        ...
        FileUtils.ensureParentDirectory(targetFile);
        DownloadUtils.ensureSha1(targetFile, AllSettings.getFastBoot().getValue() ? null : assetIndex.sha1, ()-> {
            ProgressLayout.setProgress(ProgressLayout.DOWNLOAD_MINECRAFT, 0,
                    R.string.newdl_downloading_metadata, targetFile.getName());
    ...
    private void scheduleAssetDownloads(JAssets assets) throws IOException {
        ...
        for(String asset : assetNames) {
            ...
            String sha1 = (AllSettings.getCheckLibraries().getValue() && !AllSettings.getFastBoot().getValue()) ? assetInfo.hash : null;
    ...
    private void scheduleGameJarDownload(MinecraftClientInfo minecraftClientInfo, String versionName) throws IOException {
        ...
        String clientSha1 = (AllSettings.getCheckLibraries().getValue() && !AllSettings.getFastBoot().getValue()) ?
