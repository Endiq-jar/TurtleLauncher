...
public final class JREUtils {
    ...
    private static void setJavaEnv(Map<String, String> envMap, String jreHome) {
        ...
        // Fix: on arm64 devices with ARM Memory Tagging Extension (MTE) enabled by default
        // (Pixel 8+, Galaxy S24+, and other newer SoCs), the GPU driver's native code trips
        // MTE tag-check faults and SIGSEGVs - most visibly in the Vulkan/Zink backend on the
        // latest Minecraft versions. Preloading this shim calls mallopt() to disable heap tag
        // checking for the game process before anything else runs. It's a no-op (and harmless)
        // on devices/architectures without MTE.
        File heapTaggingShim = new File(DIR_NATIVE_LIB, "libdisable_heap_tagging.so");
        if (heapTaggingShim.exists()) {
            String existingPreload = envMap.get("LD_PRELOAD");
            envMap.put("LD_PRELOAD", existingPreload == null || existingPreload.isEmpty()
                    ? heapTaggingShim.getAbsolutePath()
                    : existingPreload + ":" + heapTaggingShim.getAbsolutePath());
        }
    ...
    private static void setRendererEnv(Map<String, String> envMap) {
        ...
        if (isGl4esRenderer) {
            ...
            // TurtleLauncher: reduce libgl4es's own per-call/per-upload overhead. Batches GL
            // calls instead of dispatching each one immediately, keeps vertex data in VBOs
            // instead of client-side arrays, and skips a redundant texture copy on upload -
            // all three are real env vars in the bundled libgl4es_114/115.so, confirmed via
            // `strings` rather than assumed. Toggle exists in case a specific device/driver
            // combo regresses under batching.
            if (com.movtery.zalithlauncher.setting.AllSettings.getGl4esPerformanceTweaks().getValue()) {
                envMap.put("LIBGL_BATCH", "1");
                envMap.put("LIBGL_USEVBO", "1");
                envMap.put("LIBGL_SKIPTEXCOPIES", "1");
        ...
        boolean shaderCacheEnabled = com.movtery.zalithlauncher.setting.AllSettings.getRendererShaderCacheEnabled().getValue();

        if (isZinkRenderer) {
            envMap.put("MESA_LOADER_DRIVER_OVERRIDE", "zink");
            if (shaderCacheEnabled) envMap.put("MESA_GLSL_CACHE_DIR", PathManager.DIR_CACHE.getAbsolutePath());
            else envMap.put("MESA_GLSL_CACHE_DISABLE", "true");
        ...
        if (com.movtery.zalithlauncher.setting.AllSettings.getRendererDebugLogging().getValue()) {
            Logging.i("RendererDebug", "renderer=" + rendererId + " gl4esFamily=" + isGl4esRenderer +
                    ...
                    " gl4esPerfTweaks=" + com.movtery.zalithlauncher.setting.AllSettings.getGl4esPerformanceTweaks().getValue() +
                    " env=" + envMap);
        }
    ...
    private static void launchJavaVM(
        ...
        // TurtleLauncher CRASH FIX ATTEMPT (MC 26.3+ SDL native crash): see
        // SdlAndroidJniPrep for the full story. Only for SDL versions - a
        // no-op call for GLFW versions wouldn't help and isn't worth the risk
        // of an unnecessary side effect on versions that already work fine.
        if (gameVersion != null && Tools.versionUsesLwjglSdl(Tools.getVersionInfo(gameVersion))) {
        ...
        if (exitCode != 0) {
            ...
            if (com.movtery.zalithlauncher.setting.AllSettings.getFastBoot().getValue()) {
                com.movtery.zalithlauncher.setting.AllSettings.getFastBoot().put(false).save();
                Logger.appendToLog("Fast Boot auto-disabled after a crash, so the next launch re-verifies files.");
            }
            String diagnosis = com.movtery.zalithlauncher.feature.log.CrashAnalyzer.analyzeGameExit(gameVersion, exitCode);
    ...
    public static List<String> getJavaArgs(String runtimeHome, String userArgumentsString) {
        ...
        // ── TurtleLauncher FPS Boost Args ──────────────────────────────────────
        // Unlimited FPS mode: disables explicit GC and removes artificial frame cap
        if (com.movtery.zalithlauncher.setting.AllSettings.getUnlimitedFps().getValue()) {
            userArguments.add("-XX:+DisableExplicitGC");
            userArguments.add("-XX:+AggressiveHeap");
            userArguments.add("-Dorg.lwjgl.opengl.Display.enableHighDPI=true");
        }
        // Low latency rendering: string optimizations reduce per-frame allocation overhead
        if (com.movtery.zalithlauncher.setting.AllSettings.getLowLatencyRendering().getValue()) {
            userArguments.add("-XX:+UseStringDeduplication");
            userArguments.add("-XX:+OptimizeStringConcat");
            userArguments.add("-XX:+TieredCompilation");
        }
        // Frame pacing: LWJGL hints for smoother frame delivery on mobile
        if (com.movtery.zalithlauncher.setting.AllSettings.getFramePacing().getValue()) {
            userArguments.add("-Dorg.lwjgl.opengl.Display.noinput=false");
            userArguments.add("-Dorg.lwjgl.openal.SoftOAL=true");
        }
        // Frame skipping: drop frames on overload instead of queuing (reduces input lag)
        if (com.movtery.zalithlauncher.setting.AllSettings.getFrameSkipping().getValue()) {
            userArguments.add("-Dnet.minecraftforge.skipframes=true");
            userArguments.add("-XX:+UseAdaptiveSizePolicy");
        }
        // Adaptive frame timing: short GC pauses timed to avoid mid-frame stops
        if (com.movtery.zalithlauncher.setting.AllSettings.getAdaptiveFrameTiming().getValue()) {
            userArguments.add("-XX:MaxGCPauseMillis=10");
            userArguments.add("-XX:GCTimeRatio=9");
            userArguments.add("-XX:+UseG1GC");
        } else {
            // TurtleLauncher baseline: even without opting into Adaptive Frame Timing,
            // default to G1GC with a sane pause target instead of whatever the JRE
            // falls back to. G1 trades a little throughput for far fewer/shorter
            // GC stalls, which is what most users actually feel as "stable FPS".
            userArguments.add("-XX:+UseG1GC");
            userArguments.add("-XX:MaxGCPauseMillis=50");
        }
        // Auto memory cleanup: periodic G1 GC during idle stretches to release memory
        // back to the OS. -XX:G1PeriodicGCInterval requires -XX:+UnlockExperimentalVMOptions
        // on some JDK builds; both are added together to make sure it actually takes.
        if (com.movtery.zalithlauncher.setting.AllSettings.getAutoMemoryCleanup().getValue()) {
            userArguments.add("-XX:+UnlockExperimentalVMOptions");
            userArguments.add("-XX:G1PeriodicGCInterval=300000"); // 5 minutes
        }
        // ─────────────────────────────────────────────────────────────────────
