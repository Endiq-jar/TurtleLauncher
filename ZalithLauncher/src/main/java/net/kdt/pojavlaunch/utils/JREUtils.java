...
public final class JREUtils {
    ...
    private static void launchJavaVM(
        ...
        if (exitCode == 0) {
            // TurtleLauncher: renderer auto-fallback bookkeeping (see RendererFallbackManager) -
            // a clean exit means this renderer is fine, don't carry the "last attempt failed"
            // flag into the next launch.
            com.movtery.zalithlauncher.setting.AllSettings.getLastLaunchFailed().put(false).save();
        }
        if (exitCode != 0) {
