...
>>>>>>> 9c5f2f7990cd79a948e952a67446595d42eab51e
        ...
        // CPU Tuning is expanded; same env-var-handoff pattern as POJAV_BIG_CORE_AFFINITY
        // above (native side reads the flag, this launcher only signals intent).
        if (AllSettings.getAdvancedCpuTuning().getValue() && AllSettings.getSchedulerTuning().getValue())
            envMap.put("POJAV_SCHED_TUNING", "1");
        if (FFmpegPlugin.isAvailable)
...
>>>>>>> 9c5f2f7990cd79a948e952a67446595d42eab51e
        ...
        if (isGl4esRenderer) {
            ...
            if (com.movtery.zalithlauncher.setting.AllSettings.getJniBatching().getValue())
                envMap.put("LIBGL_BATCH", "1"); // batch GL calls instead of dispatching each one immediately
            if (com.movtery.zalithlauncher.setting.AllSettings.getJniCachedReferences().getValue())
                envMap.put("LIBGL_USEVBO", "1"); // keep vertex data in VBOs instead of re-uploading client-side arrays
            if (com.movtery.zalithlauncher.setting.AllSettings.getReducedJniCalls().getValue())
                envMap.put("LIBGL_SKIPTEXCOPIES", "1"); // skip a redundant texture copy on upload
            if (com.movtery.zalithlauncher.setting.AllSettings.getNativeObjectPooling().getValue())
        ...
        if (com.movtery.zalithlauncher.setting.AllSettings.getRendererDebugLogging().getValue()) {
            Logging.i("RendererDebug", "renderer=" + rendererId + " gl4esFamily=" + isGl4esRenderer +
                    ...
                    " jni=[batch=" + com.movtery.zalithlauncher.setting.AllSettings.getJniBatching().getValue() +
                    ",cache=" + com.movtery.zalithlauncher.setting.AllSettings.getJniCachedReferences().getValue() +
                    ",pool=" + com.movtery.zalithlauncher.setting.AllSettings.getNativeObjectPooling().getValue() +
                    ",reduced=" + com.movtery.zalithlauncher.setting.AllSettings.getReducedJniCalls().getValue() + "]" +
                    " env=" + envMap);
        }
    ...
    private static void setEnv(String jreHome, final Runtime runtime, Version gameVersion) throws Throwable {
        ...
        if (gameVersion != null) {
            ...
            // our bridge. This is a real, confirmed-in-code fix, not a guess like
            // the earlier SdlAndroidJniPrep attempt (kept in place as a harmless
            // second line of defense - see its own doc comment for why it alone
            // wasn't enough: it primes the HOST app's JNIEnv, not the separate guest
            // JVM's, so it can't register anything the guest JVM's own JNI calls
            // would ever see).
    ...
    private static void launchJavaVM(
        ...
        // Overridden below by TurtleLauncher's GC Statistics toggle, if it's on
        purgeArg(userArgs, "-Xlog:gc");

        userArgs.add("-javaagent:" + LibPath.MIO_LIB_PATCHER.getAbsolutePath());

        //Add automatically generated args
        int ramAllocationMb = AllSettings.getRamAllocation().getValue().getValue();
        ...
        boolean equalHeapSizes = com.movtery.zalithlauncher.setting.AllSettings.getEqualHeapSizes().getValue();
        ...
        int coreCount;
        int perInstanceCores = (gameVersion != null && com.movtery.zalithlauncher.setting.AllSettings.getPerInstanceCpuOverride().getValue())
                ? gameVersion.getVersionConfig().getCpuCoreOverride() : -1;
        if (perInstanceCores > 0) {
            coreCount = perInstanceCores;
        } else if (com.movtery.zalithlauncher.setting.AllSettings.getManualCoreOverride().getValue()) {
            coreCount = com.movtery.zalithlauncher.setting.AllSettings.getManualCoreCount().getValue();
        } else {
            coreCount = java.lang.Runtime.getRuntime().availableProcessors();
        }
        if (coreCount <= 0 || coreCount > 16) coreCount = 8;
        userArgs.add("-XX:ActiveProcessorCount=" + coreCount);
        ...
        if (com.movtery.zalithlauncher.setting.AllSettings.getGcStatistics().getValue()) {
