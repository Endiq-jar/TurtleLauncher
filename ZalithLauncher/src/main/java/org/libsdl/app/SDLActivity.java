...
*/
    ...
    public static void applyDroidBridgeExternalSurfaceOrientation(int width, int height) {
        if (!mDroidBridgeExternalSurfaceMode) {
            ...
            onNativeRotationChanged(rotation);
            return;
        }
         ...
         * Vulkan BufferQueue while Minecraft is rendering.
         */
        int logicalOrientation = width >= height
                ? SDL_ORIENTATION_LANDSCAPE
                : SDL_ORIENTATION_PORTRAIT;
        int reportedNatural = getNaturalOrientation();
        int reportedRotation = getCurrentRotation();
...
//        }
    ...
    public static boolean clipboardHasText() {
        if (mClipboardHandler != null) return mClipboardHandler.clipboardHasText();
        ClipboardManager manager = (ClipboardManager) getContext().getSystemService(Context.CLIPBOARD_SERVICE);
        return manager != null && manager.hasPrimaryClip();
    }
    ...
    public static String clipboardGetText() {
        if (mClipboardHandler != null) return mClipboardHandler.clipboardGetText();
        ClipboardManager manager = (ClipboardManager) getContext().getSystemService(Context.CLIPBOARD_SERVICE);
    ...
    public static void clipboardSetText(String string) {
        ...
        ClipboardManager manager = (ClipboardManager) getContext().getSystemService(Context.CLIPBOARD_SERVICE);
        if (manager != null) manager.setPrimaryClip(ClipData.newPlainText(null, string));
    }
        ...
        try
        {
            class OneShotTask implements Runnable {
                ...
                public void run() {
                    try
                    {
                        Toast toast = Toast.makeText(hostActivity, mMessage, mDuration);
                        if (mGravity >= 0) {
            ...
            hostActivity.runOnUiThread(new OneShotTask(message, duration, gravity, xOffset, yOffset));
        } catch(Exception ex) {
            return false;
        }
        return true;
