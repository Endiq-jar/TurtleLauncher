...
jint JNI_OnLoad(JavaVM* vm, __attribute__((unused)) void* reserved) {
    if (pojav_environ->dalvikJavaVMPtr == NULL) {
        ...
        // TurtleLauncher SDL3 fix: resolved here (not just when needed) for the same
        // reason as the other method_* lookups above - GetStaticMethodID must run on
        // this (the real Dalvik) JNIEnv, and this JNI_OnLoad only ever fires once for
        // that VM, so this is the one guaranteed place to do it before sdl_hook.c's
        // bytehook can possibly fire later during game launch.
