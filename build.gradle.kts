...
dependencies {
    ...
    // CI fix: was com.tencent:mmkv:2.4.1, which dropped 32-bit arch + API<23 support as of
    // v2.0.0 per its own docs - this project still ships armeabi-v7a/x86 (32-bit) builds per
    // the jniLibs ABI set, and the mismatch broke configureNdkBuildDebug on those ABIs with
    // "No compatible library found [//mmkv/mmkv]". Pinned to the 1.3.x LTS line instead, which
    // restores armeabi-v7a/x86 support (and API 21+, well under this project's minSdk 26).
